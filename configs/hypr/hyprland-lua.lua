...
--  MONITORS  --
----------------
-- HDR/10-bit variant from your old config was commented out, so this keeps the active rules only.
-- hl.monitor({ output = "DP-1", mode = "3440x1440@180", position = "0x0", scale = 1, bitdepth = 10, cm = "hdr" })
hl.monitor({
    output = "DP-1",
    mode = "3440x1440@180",
    position = "0x0",
...
-- Equivalent of legacy `exec = pkill waybar; waybar`: runs on config load/reload.
-- Your SUPER+R bind still relies on this behavior.
hl.exec_cmd("pkill waybar; waybar")

-- Equivalent of legacy `exec-once`.
...
-- Layer rules from your old decoration block.
hl.layer_rule({
    match = {
